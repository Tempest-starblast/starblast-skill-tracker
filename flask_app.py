...
def asteroids_get(seed):
    """The baked asteroid field for a map seed, or null if not baked yet.
    The client fetches this once per seed (static per match)."""
    try:
        conn = live_db()
        r = conn.execute("SELECT data FROM asteroids WHERE seed = ?",
                        (seed,)).fetchone()
        conn.close()
    except Exception:
        r = None
    if not r:
        return jsonify({"seed": seed, "asteroids": None}), 200
    try:
        return jsonify({"seed": seed, "asteroids": json.loads(r[0])}), 200
    except Exception:
        return jsonify({"seed": seed, "asteroids": None}), 200


@app.route('/api/asteroids/needed')
def asteroids_needed():
    """Seeds currently in the rawlive feed that have NOT been baked -
    the baker's work queue."""
    now = time.time()
    try:
        conn = live_db()
        seeds = set()
        for (payload,) in conn.execute("SELECT payload FROM rawlive WHERE updated > ?",
                                       (now - 120,)).fetchall():
            try:
                sd = json.loads(payload).get("seed")
                if isinstance(sd, int):
                    seeds.add(sd)
            except Exception:
                pass
        have = {r[0] for r in conn.execute("SELECT seed FROM asteroids").fetchall()}
        conn.close()
    except Exception:
        return jsonify({"needed": []}), 200
    return jsonify({"needed": sorted(seeds - have)}), 200


@app.route('/api/asteroids', methods=['POST'])
def asteroids_ingest():
    """Baker posts {seed, asteroids:[[x,y,size],...]} (radar +/-128 coords)."""
    if not api_key_ok(request.headers.get('X-API-Key')):
        return jsonify({"error": "Unauthorized"}), 401
    d = request.json
    if not isinstance(d, dict) or not isinstance(d.get("seed"), int)             or not isinstance(d.get("asteroids"), list):
        return jsonify({"error": "bad payload"}), 400
    # cap the stored size defensively
    ast = d["asteroids"][:2000]
    try:
        conn = live_db(); c = conn.cursor()
        ...
        return jsonify({"error": str(e)[:120]}), 500
    return jsonify({"ok": True, "n": len(ast)}), 200
...
def live_matches():
    ...
    for sys_id, updated, elapsed, region, name, payload in rows:
        ...
        teams = [{"key": k, "label": _lbl(k),
                  ...
                  "top": (top.get(k, "") if _owner else ""),
                  "skill": pskill.get(k) or None,
                  "players": (players_by_team.get(k) or []) if _owner else [],
...
CHANGELOG = [
    ...
    {"version": "9.13.87", "at": "2026-09-15T03:50:00Z", "changes": [
        "<b>A replay’s team lists now show who is still in the game.</b> The list is the match’s standings, so a pilot keeps the score they earned — but once they leave, their row is struck through and dimmed, instead of sitting there with no ship of theirs anywhere on the radar.",
    ]},
    ...
    {"version": "9.13.83", "at": "2026-09-15T01:00:00Z", "changes": [
        "<b>The replay clock now reads the game’s own match time.</b> A recording begins when the watcher reaches the lobby, which is not always the first second of the match — so a replay could open at “0m” with ships already upgraded and points on the board. It now opens at the real time on the match clock.",
        "Ships turn smoothly on the radar instead of snapping to each new heading.",
    ]},
    ...
    {"version": "8.5.2", "at": "2026-08-31T20:45:00Z", "changes": [
        "Replay layout: the full-height side panels (underdog on the left, favorite on the right) now spread their roster down the whole column so it fills the space beside the radar, instead of bunching the players at the top and leaving a big empty gap below."
    ]},
    ...
    {"version": "8.4.0", "at": "2026-08-31T07:30:00Z", "changes": [
        "Replay redesigned around the radar: the radar is the centerpiece, and each team gets its own panel beside it that pulls together everything about that team — its live win chance, its station, and its top players. Two teams flank the radar; three teams sit across the top with the radar below. One play bar runs it all."
    ]},
    {"version": "8.3.5", "at": "2026-08-31T06:20:00Z", "changes": [
        "Replay polish: the leaderboard shows each team's top 8 and the bars now glide smoothly instead of snapping frame to frame, the battlefield (radar + stations) spans the full width across the bottom so there's no dead space, and the radar renders properly on the left."
    ]},
    ...
    {"version": "8.0.7", "at": "2026-08-30T14:30:00Z", "changes": [
        "The match replay is now tabbed — Leaderboard, Battlefield, Charts and Result — so the whole thing fits on one screen instead of a long scroll."
    ]},
    ...
    {"version": "7.6.0", "at": "2026-08-25T03:00:00Z", "changes": [
        "The /rawlive radar now shows the asteroid field, like the good server-list bots do. Asteroids aren’t in the spectator data stream at all — they’re generated by the game from the map’s seed — so a small baker reads each map’s field straight from the game’s own generator (once per seed, since it never changes) and the radar draws it behind the ships. The map now reads like the real thing: rocks, stations and ships all in place."
    ]},
    {"version": "7.5.3", "at": "2026-08-25T01:20:00Z", "changes": [
        "Fixed radar ships appearing to fly across the map at random. The map wraps around at the edges, so a ship leaving one side comes back on the other; the view was gliding it the long way straight through the middle instead. Edge-crossings now snap to the far side like they should."
    ]},
...
def trueskill_replay_push():
    ...
    for m in ((request.json or {}).get('matches') or []):
        ...
        # multi-factor model) and full-match radar positions; store them beside
        # the frames as {"f":..,"wp":..,"rd":..}. Old pushes (frames only) stay a
        # bare list - the reader handles both.
        wp = m.get('wp')
        rd = m.get('rd')
        stl = m.get('stlay')
        sts = m.get('st')
        ...
        if isinstance(wp, list) and len(wp) == len(frames):
            obj["wp"] = wp
        if isinstance(rd, list) and len(rd) == len(frames):
            obj["rd"] = rd
        ...
        if isinstance(bs, dict) and bs.get('a'):
            obj["bs"] = bs
        # The lobby's team hues, for drawing the replay in the game's colours.
        _hues = m.get('hues')
...
def trueskill_replay_read():
    ...
    if best is not None and (best_gap is None or best_gap <= 3600):
        ...
        # The lobby's own team hues, so the replay is drawn in the colours the
...
def changelog_page():
    ...
    groups = []
    for r in CHANGELOG:
        # owner-only lines: hidden from everyone else, chipped for the owner
        _vis = []
        for _c in r.get("changes", []):
            if _c.startswith(OWNER_TAG):
                if not _owner:
                    continue
                _vis.append('<span class="ownonly">owner</span> ' + _c[len(OWNER_TAG):])
            else:
                _vis.append(_c)
        if not _vis:
            continue
        r = dict(r, changes=_vis)
        at = r.get("at")
        day = "Earlier"
        if at:
            try:
