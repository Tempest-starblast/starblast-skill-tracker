...
check("loser floor peak 100 (the scorer's, mirrored)", fa.MIN_LOCK_SCORE, 100)
_spec = open(os.path.join(ROOT, "RATING_SPEC.md"), encoding="utf-8").read()
check("the spec says the first of two ships keeps the name (24 Sep)",
      "the ship that was there first keeps the name" in _spec, True)
